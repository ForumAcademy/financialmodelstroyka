...
4. **Источники делятся на общие и проектные** (`scope: global | project`, docs/00 раздел 6). Конкретные документы проекта (ГПЗУ, ППТ, ТЭП, договор, term sheet, экспертная оценка) хранятся в проекте, в `data/sources.yaml` — только их типы. **Новый общий источник** уровня 1–3 — только с URL на первичный документ (КонсультантПлюс, Гарант, pravo.gov.ru, сайты ЦБ, Минстроя, МЭР, ФНС, ЕИСЖС). Новости и обзоры допустимы временно, с `verified: false` и `note`.
